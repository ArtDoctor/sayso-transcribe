...
:: 1. Ensure Python backend is running on port 48653
...
if errorlevel 1 (
    ...
    echo [ERROR] Python backend failed to respond on http://127.0.0.1:48653 within 20s.
    pause
    exit /b 1
)

:backend_ready
echo [OK] Python backend is active and listening on port 48653.
...
echo [*] Starting Vite frontend server at http://localhost:41765...
call npm run dev
exit /b %ERRORLEVEL%
